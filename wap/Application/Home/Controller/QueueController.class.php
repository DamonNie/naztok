...
class QueueController extends HomeController
{
	...
	public function upTrade($market = NULL)
	{
		...
		//echo $max_num."<br/>";
		//echo $min_num."<br/>";
		//echo rand($min_num/10000, $max_num/10000);
		//echo $num;
		//die();
			...
			if ($type == 2) {
				...
				if ($bili_user) {
					...
					if ($bili_keyi) {
						...
						if ($bili_kegua < $num) {
							echo  '您的挂单总数量超过系统限制，您当前持有' . C('coin')[$xnb]['title'] . $bili_user . '个，已经挂单' . $bili_zheng[0]['nums'] . '个，还可以挂单' . $bili_kegua . '个';
						}
					}
					else {
						echo  '可交易量错误';
		...
		if (C('market')[$market]['trade_min']) {
			if ($mum < C('market')[$market]['trade_min']) {
				echo  '交易总额不能小于' . C('market')[$market]['trade_min'];
			}
		}

		if (C('market')[$market]['trade_max']) {
			if (C('market')[$market]['trade_max'] < $mum) {
				echo  '交易总额不能大于' . C('market')[$market]['trade_max'];
			}
		}
